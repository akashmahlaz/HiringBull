      ...
      try {
        ...
        if (cachedCompanies && cachedTimestamp) {
          ...
          if (timeDiff < CACHE_DURATION) {
            try {
              const data = toArray(JSON.parse(cachedCompanies));
              if (data.length === 0) throw new Error('Invalid cached companies');
        ...
        const fullUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/companies`;
    ...
    const fetchAllJobs = async () => {
      try {
        ...
        if (cachedJobs && cachedTimestamp) {
          ...
          if (timeDiff < CACHE_DURATION) {
            try {
              const data = toArray(JSON.parse(cachedJobs));
              if (data.length === 0) throw new Error('Invalid cached jobs');
        ...
        while (hasMorePages) {
          const jobsEndpoint = `${process.env.NEXT_PUBLIC_API_URL}/api/free-jobs?page=${page}&limit=${jobsPerPage}`;
      ...
      <MainContent>
        <MemberNotice>
          ...
          <NoticeText>
            ...
            <UpgradeLink as={Link} href="/join-membership">Upgrade to Premium</UpgradeLink> for real-time job updates and exclusive listings.
          </NoticeText>
        </MemberNotice>
