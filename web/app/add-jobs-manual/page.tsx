...
const AddJobs = () => {
  ...
  const fetchCompanies = async (force = false) => {
    try {
      ...
      if (!force) {
        ...
        if (cached) {
          try {
            const cachedCompanies = toArray(JSON.parse(cached));
            ...
            console.error("Invalid cached companies", cacheError);
          }
  ...
  const filteredCompanies = toArray(companies).filter(c =>
