  ...
  const fetchJobsFromAPI = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/free-jobs?page=1&limit=50`
      );
  ...
  const loadJobs = () => {
    ...
    if (cachedJobs && cachedTimestamp) {
      ...
      if (timeDiff < CACHE_DURATION) {
        try {
          const jobsArray = toArray(JSON.parse(cachedJobs));
          if (jobsArray.length === 0) throw new Error('Invalid cached jobs');
          setJobs(jobsArray);
